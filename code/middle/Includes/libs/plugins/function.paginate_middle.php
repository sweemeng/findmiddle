...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    $_prefix = '';
    $_suffix = '';
    $_link_prefix = '<span class="text12page pagenum">';
    $_link_suffix = '</span>'; 
    $_page_limit = null;
    $_attrs = array();
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            // Below custom hack to get the display going correct; yuk!!
            $_ret .= '<a href="' . str_replace('&', '&amp;', $_this_url) . '" class="text12page pagenum">' . $_text . '</a>';
        } else {
